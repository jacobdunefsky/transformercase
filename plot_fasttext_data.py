...
for c_short in cases_short:
	...
	for cur_short in cases_short:
		...
		pathname = f"ft_data/ft-{c_short}-{cur_short}.pkl"
		if not os.path.exists(pathname):
			pathname = f"ft_data/ft-{cur_short}-{c_short}.pkl"
		datas[cur_short] = np.asarray(pickle.load(open(pathname, "rb")))

	nom_all = np.asarray(pickle.load(open(f"ft_data/ft-{c_short}-all.pkl", "rb")))
