...
for c_short in cases_short:
	...
	for cur_short in cases_short:
		...
		pathname = f"tf_data/{c_short}-{cur_short}.pkl"
		if not os.path.exists(pathname):
			pathname = f"tf_data/{cur_short}-{c_short}.pkl"
		datas[cur_short] = np.asarray(pickle.load(open(pathname, "rb")))

	nom_all = np.asarray(pickle.load(open(f"tf_data/{c_short}-all.pkl", "rb")))
	nom_rnd = np.asarray(pickle.load(open(f"tf_data/{c_short}-random.pkl", "rb")))
